unit AtmtApp.CalendarOutlook.old;
...
type
    ...
    protected   //public via interface
        ...
        //ICalendarApp
        function GetAppointmentsList(): ICalAppmtList; override;
    end;
...
type
    ...
    private
        //FOutlookCalendar: OleVariant;
        FItems: OleVariant;
    protected   //public via interface
        function Count(): Integer;
        function GetItem(AIndex: Integer): ICalAppmt;
    public
        constructor Create(AOutlookCalendar: OleVariant);
    end;
    ...
    protected   //public via interface
        ...
        class function CalcGlobalUniqueId(const AGlobalAppointmentId: string; const AStartDate: TDateTime): string;
    public
...
begin
    ...
    Result := TIOutlookCalAppmt.Create(FOutlookCalendar.Items[AIndex + 1]);      //our AIndex is zero based, outlook is 1-based
end;
...
begin
    Result := TIOutlookCalAppmt.CalcGlobalUniqueId(PropGetGlobalId, PropGetStartTime);
end;
...
class function TIOutlookCalAppmt.CalcGlobalUniqueId(const AGlobalAppointmentId: string;
  const AStartDate: TDateTime): string;
var
...
begin
    ...
    GlobalIdBytes := HexStringToBytes(AGlobalAppointmentId);

    // Format the date as 'yyyymmddhhnnss'
    FormattedDate := FormatDateTime('yyyymmddhhnnss', AStartDate);

    // Encode the byte array using Soap.EncdDecd.EncodeBase64
    ByteStream := TBytesStream.Create(GlobalIdBytes);
    try
        EncodedId := EncodeBase64(ByteStream, ByteStream.Size);
    finally
        ByteStream.Free;
    end;
...
function TIOutlookCalendarApp.GetAppointmentsList: ICalAppmtList;
var
...
begin
    ...
    Result := TIOutlookCalAppmtList.Create(Calendar);
end;
